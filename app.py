...
def transcribe_reference(audio_path, mode_input, language="English"):
    ...
    elif isinstance(mode_input, str) and "High-Quality" in mode_input:
        should_run = True

    if not audio_path or not should_run:
        return gr.update()
...
def generate_voice_design(text, language, voice_description, remove_silence, make_subs):
    if not text or not text.strip():
        return None, "Error: Text is required.", None, None, None, None
...
def smart_generate_clone(
    ...
    if not ref_audio:
        return None, "Error: Ref audio required.", None, None, None, None
    ...
    try:
        ...
        for i, chunk in enumerate(text_chunks):
            wavs, sr = tts.generate_voice_clone(
                ...
                voice_clone_prompt=final_ref_text.strip() if final_ref_text else None,
        ...
        with gr.Tabs():
            ...
            with gr.Tab("Voice Clone (Base)"):
                ...
                clone_ref_audio.change(
                    ...
                    inputs=[clone_ref_audio, clone_mode, clone_language],
